...
@app.cell
def _(get_kp, np, stab_kps):
    def compute_root_motion(stab_list, target_spine=200.0, detrend_win=31, bounce_gain=0.6):
        pel = []
        for fr in stab_list:
            neck = get_kp(fr, 'Neck'); tail = get_kp(fr, 'root_of_tail')
...
def _(
    ...
    def compute_tail_rel():
        rels = []
        for fr, tip in zip(stab_kps, tail_tip_track):
            neck = get_kp(fr, 'Neck'); tail = get_kp(fr, 'root_of_tail')
            d_body = tail - neck
            d_tail = np.asarray(tip, dtype=float) - tail
            if np.linalg.norm(d_body) < 1e-6 or np.linalg.norm(d_tail) < 1e-6:
                rels.append(rels[-1] if rels else 0.0)
                continue
            rels.append(wrap_angle(np.arctan2(d_tail[1], d_tail[0])
                                   - np.arctan2(d_body[1], d_body[0])))
        return np.asarray(rels)
    ...
    def solve_pose(frame_data, rig, idx, root_y=0.0, tail_state=None,
        ...
        w['tail_1'] = wrap_angle(base_dir + np.pi + rel)
