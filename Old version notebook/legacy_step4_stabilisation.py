...
def _(cv2, inferencer_s4, json):
    ...
    # Video comes from POSETOON_VIDEO when the pipeline notebook drives this
    # file, so twenty videos need no edit here. Falls back to the original
    # clip when the file is opened on its own.
    import os as _os4
    _VIDEO = _os4.environ.get('POSETOON_VIDEO', 'dogvideo.mp4')
    print(f"extracting from {_VIDEO}")
    full_video_kps = extract_full_video_keypoints(_VIDEO, inferencer_s4)
...
def _(full_video_kps, json, np, one_euro_filter_fn):
    ...
    # NOTE -- superseded by 2_repair_keypoints.py, which does the same job
    # correctly (duplicate-frame retiming + kinematic outlier rejection)
    # and writes the same stabilised_keypoints.json. This cell also has a
    # latent bug: apply_stabilisation_all_joints() appends to
    # `stabilised_kps` before it is created, so it raises NameError on a
    # fresh run. Only the EXTRACTION cell above is needed from this file.
