...
def _(
    ...
    _sl_rel = _sl_raw / np.maximum(_movmed(_sl_raw), 1e-6) - 1.0
    ...
    def _arch_of(track_al):
        ...
        for i_al, fr_al in enumerate(stab_kps):
            neck_al = get_kp(fr_al, 'Neck')
            ch_al = get_kp(fr_al, 'root_of_tail') - neck_al
            cl_al = float(np.linalg.norm(ch_al))
            if cl_al < 1e-6:
                continue
            u_al = ch_al / cl_al
            out_al[i_al] = float((np.asarray(track_al[i_al], dtype=float)
                                  - neck_al)
    ...
    if ANNOT_ALIGN_AUTO:
        ...
        _chd_dc = chord_bend_series - float(np.mean(chord_bend_series))
        ...
        print(f"annotation vs gait-locked chord bend: corr {_r_zero:+.2f} at "
    ...
    BEND_FULL_SCALE = (ARCH_LIMIT_POS * SPINE_ARCH_GAIN
                       + CHORD_BEND_WEIGHT
    ...
    print(f"combined bend full-scale = {BEND_FULL_SCALE:.3f} rad "
          f"(annotation {ARCH_LIMIT_POS * SPINE_ARCH_GAIN:.3f} + chord "
    ...
    def solve_pose(frame_data, rig, idx, root_y=0.0, tail_state=None,
        ...
        if chord_len > 1e-6:
            ...
            arch = float(_a) * float(arch_trust_series[idx])
        else:
            arch = 0.0
        ...
        anchors = {
            'shoulder': edge_anchor(_chain_point(SH_ALONG, SH_PERP),
                                    spine_verts[3], w['spine4']),
            'hip':      edge_anchor(_chain_point(HP_ALONG, HP_PERP),
