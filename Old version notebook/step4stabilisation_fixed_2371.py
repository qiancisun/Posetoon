...
def _(full_video_kps, np, plt):
    ...
    def one_euro_filter_fn(data, min_cutoff=1.0, beta=0.007):
        """Offline, zero-phase One Euro smoothing to avoid delayed motion."""
        data_arr = np.array(data, dtype=float)
        if len(data_arr) < 2:
            return data_arr
        ...
        def one_pass(values):
            ...
            for i in range(1, len(values)):
                ...
                result[i] = x_prev + alpha * (values[i] - x_prev)
                x_prev = result[i]
                dx_prev = dx_smooth
            return result

        forward = one_pass(data_arr)
        backward = one_pass(data_arr[::-1])[::-1]
