...
def _(
    ...
    def repair_and_stabilise_v2(score_thr=0.3):
        ...
        # v4 -- kinematic outlier rejection. Confidence thresholds cannot
        # catch a CONFIDENT-but-wrong detection: measured on this clip,
        # single raw keypoints jump 163-175px in one frame with healthy
        # scores (drawn dots at f58/f63 in the skeleton panel), and such
        # frames sail through every score-based filter to become 1-frame
        # limb twitches in the render. Detection is kinematic instead: a
        # sample sitting far from the midpoint of its two neighbours has a
        # near-impossible acceleration. An isolated garbage sample scores
        # ~the full jump; a REAL paw reversal scores only accel/2 (~5-10px),
        # so the two separate by an order of magnitude and real motion is
        # not eaten -- the failure mode this project has hit before.
        HAMPEL_K = 6.0             # x MAD of the joint's own residuals
        HAMPEL_MIN_PX = 14.0       # absolute floor, above real reversals

        for j in range(num_joints):
            # v3 -- sub-threshold detections must NOT contaminate the pair
            # average. With w = clip(score, 1e-6) a garbage detection at
            # score 0.25 still pulled its score-0.6 partner by weight 0.25,
            # which produced the isolated 1-frame spikes seen at ~f137/f207
            # after the v2 repair. Zero weight below score_thr; a group whose
            # detections are ALL below threshold gets uniq_sc < score_thr and
            # is gap-filled by interpolation as before.
            ...
            # 2. gap fill on the unique timeline, in two passes: score-based
            # bads are provisionally filled first so a garbage group cannot
            # distort its neighbours' kinematic residuals, then the outlier
            # test runs on the filled series, and the union is filled again.
