...
def _(mo):
    mo.md(r"""
    ## Running the upstream stages

    This notebook does the rigging and rendering. The four stages before it — keypoint extraction, repair, coat sampling and template selection — are separate scripts, and the cell below runs any that have not already written their output.

    Set `RUN_UPSTREAM = False` to work from files already on disk, or `FORCE_RERUN = True` to redo them.
    """)
    return
...
def _():
    ...
    # run_all.py drives this file for a batch: it has already run the four

    VIDEO_PATH = os.environ.get("POSETOON_VIDEO", VIDEO_PATH)
    if os.environ.get("POSETOON_RUN_UPSTREAM") == "0":
        RUN_UPSTREAM = False

    STAGES = [
    
        ("1_extract_keypoints.py", True,  "full_video_keypoints.json",
         "MMPose keypoint extraction"),
        ("2_repair_keypoints.py",  True,  "stabilised_keypoints.json",
         "duplicate-frame repair + outlier rejection"),
        ("3_coat_colour.py",       False, "outputs/coat_palette.json",
         "coat colour sampling"),
        ("4_select_breed.py",      False, "outputs/breed_selection.json",
         "breed template selection"),
    ]

    def run_upstream(video, force=False):
        if not os.path.exists(video):
            raise FileNotFoundError(
                f"{video} not found. Put the clip beside this notebook, or "
                f"set VIDEO_PATH to its path.")
        env = dict(os.environ, POSETOON_VIDEO=video)
        for script, is_marimo, output, label in STAGES:
            if os.path.exists(output) and not force:
                print(f"[skip] {label}  ({output} exists)")
                continue

            cmd = [sys.executable, script] + ([] if is_marimo else [video])
            print(f"[run ] {label}  ->  {output}")
            t0 = time.time()
            ...
            if r.returncode != 0:
                ...
                print(r.stderr[-3000:])
                raise RuntimeError(
                    f"{script} failed (exit {r.returncode}). Its output is "
                    f"above; fix that before the rig can run.")
            ...
            if not os.path.exists(output):
                raise RuntimeError(
                    f"{script} finished but did not write {output}.")
            print(f"        done in {time.time() - t0:.0f}s")

    if RUN_UPSTREAM:
        run_upstream(VIDEO_PATH, force=FORCE_RERUN)
        print("\nupstream complete -- rigging below")
    else:
        print("RUN_UPSTREAM is off; using the files already on disk")
    return (VIDEO_PATH,)
...
def _(mo):
    mo.md(r"""
    ## Inputs and keypoint conventions

    Loads the tracked skeleton and the measurements taken from it, and fixes the naming used everywhere below. Keypoints follow the AP-10K order: eyes, nose, neck, root of tail, then shoulder–elbow–paw for each foreleg and hip–knee–paw for each hind leg.

    `get_kp` and `get_score` are the only places that touch the raw arrays, so a change in the tracked format is contained here.
    """)
    return
...
def _():
    ...
    spine_mid_track = None
    try:
        with open('spine_tail_tracks.json', 'r') as f:
            spine_tail = json.load(f)
        _sm = np.array(spine_tail['spine_mid'], dtype=float)   # (N,2)
        _tt = np.array(spine_tail['tail_tip'], dtype=float)    # (N,2)
        if len(_sm) == len(stab_kps) and len(_tt) == len(stab_kps):
            spine_mid_track, tail_tip_track = _sm, _tt
            SPINE_TAIL_SOURCE = 'annotated'
        else:
            print(f"  !! spine_tail_tracks.json has {len(_sm)} frames but this "
                  f"clip has {len(stab_kps)}.")
            print(f"     The annotation belongs to a different video; ignoring "
                  f"it and using the fallback.")
    except FileNotFoundError:
        pass

    if spine_mid_track is None:
    ...
    if SPINE_TAIL_SOURCE == 'synthesised':
        print("  Running the fallback path (no usable annotation for this clip).")
...
def _(mo):
    ...
    Turns the tracked skeleton into the handful of numbers the character is built from: spine length, head length, and the four limb segments.

    Frames where a keypoint is not confidently seen are dropped rather than trusted, and each bone is taken as a percentile over the remaining frames — a moving animal is foreshortened in most frames, so a percentile of many frames describes it better than any single one.
    """)
    return
...
def _(get_kp, get_score, np, stab_kps):
    ...
    MIN_CONFIDENT_FRAMES = 12

    LIMB_PERCENTILE = 50

    def measure_skeleton(stab_list, target_spine=200.0):
        acc = {k: [] for k in ['lower', 'upper', 'head', 'hum', 'rad', 'fem', 'tib']}
        used = {k: 0 for k in acc}
        n_seen = 0

        def ok(fr, *names):
            return all(get_score(fr, n) >= MIN_KP_SCORE for n in names)

        def seg(fr, a, b, sc):
            """Longer of the two sides, counting only the sides that are
            confidently seen. None when neither side is."""
            out = []
            for side in ('L', 'R'):
                na, nb = a.format(s=side), b.format(s=side)
                if ok(fr, na, nb):
                    out.append(np.linalg.norm(get_kp(fr, nb) - get_kp(fr, na)) * sc)
            return max(out) if out else None

        for fr in stab_list:
            neck = get_kp(fr, 'Neck'); tail = get_kp(fr, 'root_of_tail')
            sl = np.linalg.norm(neck - tail)
            if sl < 1e-6 or not ok(fr, 'Neck', 'root_of_tail'):
                continue
            n_seen += 1
            sc = target_spine / sl
            ...
            for key, a, b in (('hum', '{s}_Shoulder', '{s}_Elbow'),
                              ('rad', '{s}_Elbow', '{s}_F_Paw'),
                              ('fem', '{s}_Hip', '{s}_Knee'),
                              ('tib', '{s}_Knee', '{s}_B_Paw')):
                v = seg(fr, a, b, sc)
                if v is not None:
                    acc[key].append(v)
        ...
        for k in acc:
            used[k] = len(acc[k])
            if len(acc[k]) < MIN_CONFIDENT_FRAMES:
                fallback.append(k)
        if fallback:
            for fr in stab_list:
                neck = get_kp(fr, 'Neck'); tail = get_kp(fr, 'root_of_tail')
                ...
                if 'head' in fallback:
                    acc['head'].append(np.linalg.norm(get_kp(fr, 'Nose') - neck) * sc)
                for key, a, b in (('hum', 'L_Shoulder', 'L_Elbow'),
                                  ('rad', 'L_Elbow', 'L_F_Paw'),
                                  ('fem', 'L_Hip', 'L_Knee'),
                                  ('tib', 'L_Knee', 'L_B_Paw')):
                    if key in fallback:
                        acc[key].append(
                            np.linalg.norm(get_kp(fr, b) - get_kp(fr, a)) * sc)

        m = {}
        for k in ['lower', 'upper', 'head']:
            m[k] = float(np.median(acc[k]))          # head included
        for k in ['hum', 'rad', 'fem', 'tib']:
            m[k] = float(np.percentile(acc[k], LIMB_PERCENTILE))

        print(f"measured proportions (spine = 200), from {n_seen}/"
              f"{len(stab_list)} frames with a confident spine:")
        for k in ['lower', 'upper', 'head', 'hum', 'rad', 'fem', 'tib']:
            tag = ""
            if k in ('head', 'hum', 'rad', 'fem', 'tib'):
                tag = f"   [{used[k]} confident frame(s)]"
                if k in fallback:
                    tag += "  <- too few, fell back to unfiltered left side"
            print(f"  {k:6}: {m[k]:6.1f}px{tag}")
        _leg = 0.5 * ((m['hum'] + m['rad']) + (m['fem'] + m['tib'])) / target_spine
        print(f"  leg-to-spine {_leg:.3f}")
        return m
...
def _(mo):
    mo.md(r"""
    ## Choosing a template and building the rig

    The measured proportions are matched against twelve breed templates and blended with the chosen one. `alpha` controls that blend: 0 is the template untouched, 1 is this dog's own measurements.

    The four force and complex switches at the top override the automatic choices and exist for the ablation experiments.
    """)
    return
...
def _(SPINE_TAIL_SOURCE, measure, np, stab_kps):
    def build_rig(measure, ref=200.0):
        ...
        for side, base, cu, cm, cl in [('L', 40, '#444444', '#3C3C3C', '#2A2A2A'),
            ...
            # Lateral offset between the left and right limb roots.
    ...
    FORCE_TEMPLATE = None    # Labrador/ 'Whippet  (E2 conditions)
    FORCE_ALPHA = None       # pin blend strength
    COMPLEXITY = 'fine'      # finecoarse'            (E3 conditions)
...
def _(mo):
    ...
    The whole character rises and falls as the dog moves. Taking that from the tracked body directly is too noisy to use, so the vertical signal is filtered before it drives the root.
    """)
    return
...
def _(mo):
    ...
    Builds one angle track per limb segment from the tracked joints.

    Low-confidence frames are interpolated across rather than used, and the result is smoothed with a One Euro filter, which follows fast motion closely while still settling during slow motion.
    """)
    return


@app.cell
def _(aline_info, facing_series, get_kp, get_score, measure, np, stab_kps):
    ...
    def build_leg_tracks(stab_list, facing_arr):
        ...
        for i, fr in enumerate(stab_list):
            ...
            for leg, (rt, md, tp, _, _) in LEG_DEF.items():
                ...
                conf[leg][i] = cmin
        _leg_used = aline_info['measure_used']['fem'] + aline_info['measure_used']['tib']
        _leg_meas = measure['fem'] + measure['tib']
        LEG_RATIO = float(np.clip(_leg_used / max(_leg_meas, 1e-6), 1.0, 1.5))
        ACCEL_TARGET_DEG = 6.0 / LEG_RATIO
        print(f"leg length ratio {LEG_RATIO:.3f} -> angular accel target "
              f"{ACCEL_TARGET_DEG:.2f} deg (6.00 at measured length)")
        ...
        COLLAPSE_FRAC = 0.045          
        COLLAPSE_ANGLE = 0.10          # below this the limbs are parallel
        spine_px = np.array([
        ...
        for lname, rname, jl, jr in (
            ...
            merged = (sep < COLLAPSE_FRAC) & (d_up < COLLAPSE_ANGLE) \
                & (d_lo < COLLAPSE_ANGLE)
            # demote whichever side the estimator is less sure of
        ...
        def conf_fill(v, c):
            ...
            wgt = wgt * wgt * (3.0 - 2.0 * wgt)        # smoothstep, no corners
...
def _(mo):
    mo.md(r"""
    ## Which way the dog faces

    Facing is the sign of the nose's position relative to the root of the tail. It only flips after several consecutive frames agree, so a couple of noisy frames cannot mirror the whole character mid-stride.
    """)
    return
...
def _(mo):
    ...
    The largest section. For every frame it turns the tracked angles into the rig's joint angles: spine arch and stretch, limb angles against their anatomical limits, tail, and the lateral spread that separates the near and far legs.

    The constants at the top are the tuned ones — joint limits, arch gain, smoothing widths — and each is set from a measurement rather than by eye.
    """)
    return
...
def _(
    ...
    # smoothed tail angle, precomputed over the whole clip 
    ...
    if USE_MEASURED_STRETCH:
        ...
        _share_k = 3.0 * _u_k ** 2 - 2.0 * _u_k ** 3      # smoothstep
        _contr = -_share_k * np.maximum(-_sl_rel, 0.0)
        _r_bend = np.clip(1.0 + BEND_SHARE * _contr, 0.30, 1.0)
        # invert sin(x)/x = r by Newton from the 2nd-order seed
        ...
        HOLLOW_GAIN = 1.6          # radians of hollow per unit chord excess
    ...
    # limb anchor series 
    _ANCHOR_CONF = 0.35        # same threshold the leg tracks use
    ...
    if ANNOT_ALIGN_AUTO:
        ...
        MIN_ALIGN_CORR = 0.30
        MIN_GAIT_RANGE = 0.08      # peak to peak limb spread, body lengths
        _gait_range = float(np.ptp(_spread_al))

        _synth = (SPINE_TAIL_SOURCE == 'synthesised')
        _ann_flat = _synth or float(np.std(_ann_dc)) < 1e-9

        print(f"annotation vs {_ref_name}: corr {_r_zero:+.2f} at "
              f"lag 0, best {_r_best:+.2f} at lag {_lag_best:+d}"
              f"   [gait range {_gait_range:.3f}]")

        if _ann_flat:
            print("   => annotation carries no arch (synthesised track); "
                  "nothing to re-time" if _synth else
                  "   => annotation carries no arch; nothing to re-time")
        elif _gait_range < MIN_GAIT_RANGE:
            print(f"   => limb spread varies by only {_gait_range:.3f} body "
                  f"lengths; too little gait to align against, left as is")
        elif not np.isfinite(_r_best) or _r_best < MIN_ALIGN_CORR:
            print(f"   => best correlation {_r_best:+.2f} is below "
                  f"{MIN_ALIGN_CORR}; a lag fitted this weakly would be "
                  f"fitted to noise, left as is")
        elif abs(_lag_best) >= 3 and _r_best > _r_zero + 0.15:
    ...
    CHORD_GATE_LO, CHORD_GATE_HI = 0.25, 0.55
    ANNOT_CHANNEL_GAIN = 0.0 if SPINE_TAIL_SOURCE == 'synthesised' else 1.0
    _r_gate = _r_chd if 'np' in dir() and np.isfinite(_r_chd) else 0.0
    CHORD_CHANNEL_GAIN = float(np.clip(
        (_r_gate - CHORD_GATE_LO) / (CHORD_GATE_HI - CHORD_GATE_LO), 0.0, 1.0))
    print(f"flexion channel gates: annotation x{ANNOT_CHANNEL_GAIN:.2f} "
          f"({SPINE_TAIL_SOURCE}), chord x{CHORD_CHANNEL_GAIN:.2f} "
          f"(corr {_r_gate:+.2f} vs limb gathering, "
          f"full weight above {CHORD_GATE_HI})")
    if CHORD_CHANNEL_GAIN < 0.999:
        print("   the chord shortening on this clip is not locked to the "
              "gait, so it is not read as spinal flexion; the back will be "
              "close to rigid.")
    ...
    BEND_FULL_SCALE = (ARCH_LIMIT_POS * SPINE_ARCH_GAIN * ANNOT_TRUST_SCALE
                       * ANNOT_CHANNEL_GAIN
                       + CHORD_BEND_WEIGHT * CHORD_CHANNEL_GAIN
    ...
    print(f"combined bend full-scale = {BEND_FULL_SCALE:.3f} rad "
          f"(annotation "
          f"{ARCH_LIMIT_POS * SPINE_ARCH_GAIN * ANNOT_TRUST_SCALE * ANNOT_CHANNEL_GAIN:.3f}"
          f" + chord "
          f"{CHORD_BEND_WEIGHT * CHORD_CHANNEL_GAIN * float(np.percentile(chord_bend_series[_obs], 95)) / 2.25:.3f})")
    ...
    # Mid-tail whip, from tail_1's own angular acceleration.
    ...
    print(f"ear (raw, before the template's ear_flop scaling): "
          f"lag +-{np.abs(ear_lag_series).max()*180/np.pi:.1f} deg, "
    ...
    def solve_pose(frame_data, rig, idx, root_y=0.0, tail_state=None,
        ...
        # datadriven arch from hand-tracked back mid
        ...
        total_bend = (SPINE_REST_FOLD + arch * SPINE_ARCH_GAIN
                      + CHORD_BEND_WEIGHT * CHORD_CHANNEL_GAIN   # FIX Q
        ...
        _flop = float(rig.get('_style', {}).get('ear_flop', 1.0))
        _el = float(ear_lag_series[idx]) * facing * _flop
        _ed = float(ear_drag_series[idx]) * _flop
        _ear = w['head'] + rig['_style']['ear_droop'] + _el + _ed


        _lock = float(rig.get('_style', {}).get('ear_world_lock', 0.0))
        if _lock > 0.0:
            _d = (-np.pi / 2) - _ear
            _d = (_d + np.pi) % (2 * np.pi) - np.pi      # shortest way round
            _ear = _ear + _lock * _d
        w['ear_upper'] = _ear


        w['ear_lower'] = (w['ear_upper'] + 0.25 * _flop
                          + _el * 0.55 + _ed * 0.4)
        w['shoulder'] = w['spine4']
        w['hip'] = w['spine1']
        ...
        TAIL_SET_ANNOTATED_GAIN = 0.35
        rel = tail_rel_series[idx] * facing
        tail_base = base_dir + TAIL_FOLLOW_RUMP * (w['spine1'] - base_dir)

        _st = rig.get('_style', {})
        _set_gain = (1.0 if SPINE_TAIL_SOURCE == 'synthesised'
                     else TAIL_SET_ANNOTATED_GAIN)
        _carry = float(_st.get('tail_set', 0.0)) * _set_gain
        _curl_base = float(_st.get('tail_curl_base', 0.0))
...
def _(mo):
    ...
    Walks the bone hierarchy to give every part a position and rotation on screen for a given frame.
    """)
    return
...
def _(mo):
    ...
    Draws the parts in depth order: far limbs behind the body, then the torso, then the near limbs, head and tail.

    The torso is a ribbon that follows the solved spine, so it bends with the animal instead of being a rigid block. Long-coated breeds get a broken outline; short-coated ones do not.
    """)
    return
...
def _(Image, ImageDraw, np, rig):
    ...
    def _dark(hex_colour, dl):
        h = hex_colour.lstrip('#')
        r, g, b = [int(h[i:i + 2], 16) / 255 for i in (0, 2, 4)]
        _h, _l, _s = _cs.rgb_to_hls(r, g, b)
        _r, _g, _b = _cs.hls_to_rgb(_h, max(0.02, min(0.98, _l - dl)), _s)
        return '#{:02X}{:02X}{:02X}'.format(int(_r * 255), int(_g * 255),
                                            int(_b * 255))

    _jp = rig.get('_style', {}).get('palette', {})

    def _joint(part, fallback, dl_fill, dl_line):
        base = _jp.get(part, fallback)
        return _dark(base, dl_fill), _dark(base, dl_line)

    _n_f, _n_o = _joint('neck', '#4A4A4A', 0.06, 0.14)
    _h_f, _h_o = _joint('head', '#4A4A4A', 0.06, 0.14)
    _nu_f, _nu_o = _joint('near_upper', '#505050', 0.07, 0.16)
    _nm_f, _nm_o = _joint('near_mid', '#4A4A4A', 0.07, 0.16)
    _fu_f, _fu_o = _joint('far_upper', '#5E5E5E', 0.05, 0.11)
    _fm_f, _fm_o = _joint('far_mid', '#5A5A5A', 0.05, 0.11)

    JOINT_STYLE = {
        'neck': (0.022, _n_f, _n_o, 2), 'head': (0.016, _h_f, _h_o, 1),
        'humerus_L': (0.040, _nu_f, _nu_o, 2), 'radius_L': (0.027, _nm_f, _nm_o, 2),
        'femur_L': (0.038, _nu_f, _nu_o, 2), 'tibia_L': (0.027, _nm_f, _nm_o, 2),
        'humerus_R': (0.036, _fu_f, _fu_o, 1), 'radius_R': (0.025, _fm_f, _fm_o, 1),
        'femur_R': (0.034, _fu_f, _fu_o, 1), 'tibia_R': (0.025, _fm_f, _fm_o, 1),
    }
    ...
    def body_ribbon(pose):
        ...
        w_up_k = np.asarray(meta.get(
            'w_up_k', np.array([0.112, 0.128, 0.132, 0.128, 0.112]) * 200.0),
            dtype=float)
        w_dn_k = np.asarray(meta.get(
        ...
        _fur = float(rig.get('_style', {}).get('fur_amplitude', 0.0))

        def _run(points, norms, widths, sign):
            """Resample one edge of the ribbon and displace it outward."""
            if _fur <= 1e-6 or len(points) < 2:
                return [tuple(p) for p in points]
            out = []
            n_seg = len(points) - 1
            for i in range(n_seg):
                for k in range(FUR_SUBDIV):
                    t = k / float(FUR_SUBDIV)
                    p = points[i] * (1 - t) + points[i + 1] * t
                    nm = norms[i] * (1 - t) + norms[min(i + 1, len(norms) - 1)] * t
                    wd = widths[i] * (1 - t) + widths[min(i + 1, len(widths) - 1)] * t
                    u = (i + t) / max(1.0, n_seg)          # 0..1 along the run
                    # Two incommensurate frequencies: one gives the tufts,
                    # the other stops them looking like a regular sawtooth.
                    r = (np.sin(u * 21.0 + sign * 1.7)
                         + 0.55 * np.sin(u * 47.0 + sign * 4.1)
                         + 0.25 * np.sin(u * 8.0 + sign * 0.6))
                    out.append(tuple(p + nm * (sign * abs(r) * _fur * wd)))
            out.append(tuple(points[-1]))
            return out

        _n_up = [-normals[i] for i in range(len(verts))]
        _n_dn = [normals[i] for i in range(len(verts))]

        poly = []
        poly.append(tuple(rump_top))
        poly += _run(np.array(top), np.array(_n_up), np.array(w_up), 1.0)
        poly.append(tuple(neck_ext - normals[-1] * w_up[-1]))
        poly.append(tuple(neck_ext + normals[-1] * w_dn[-1]))
        poly += _run(np.array(bot[::-1]), np.array(_n_dn[::-1]),
                     np.array(w_dn[::-1]), 1.0)
    ...
    def render_rig(rig, world, pose=None, facing=1.0, canvas_size=(600, 400), ref=200.0):
        ...
        def _stroke(poly_pts, fill, line, width):
            if fill:
                d.polygon(poly_pts, fill=fill)
            if line and width > 0:
                pts = list(poly_pts) + [poly_pts[0]]
                d.line(pts, fill=line, width=int(width), joint='curve')

        def _bone_w(nm, default=1):
            b = rig.get(nm, {})
            try:
                return int(b.get('line_w', default))
            except (AttributeError, TypeError, ValueError):
                return default


        RIBBON_LAYER = 25
        FAR_LIMBS_BEHIND_BODY = True
        _behind = {n for n, b in rig.items()
                   if not n.startswith('_') and b.get('mesh') is not None
                   and b['layer'] < RIBBON_LAYER
                   and (FAR_LIMBS_BEHIND_BODY or n.startswith('tail'))}
        for name in sorted(_behind, key=lambda n: rig[n]['layer']):
            if name not in world:
                continue
            b = rig[name]
            poly_b = to_world(b['mesh'], world[name])
            _stroke(poly_b, b['color'], b['outline'], _bone_w(name))

        # body ribbon 
        if pose is not None and 'spine_verts' in pose:
            poly = body_ribbon(pose)
            _pal = rig.get('_style', {}).get('palette', {})
            _stroke([(int(x), int(y)) for (x, y) in poly],
                    _pal.get('body', '#3D3D3D'),
                    _pal.get('outline', '#222222'),
                    rig.get('_style', {}).get('line_w_body', 1))
        drawable = [(n, b) for n, b in rig.items()
                    if not n.startswith('_') and b.get('mesh') is not None
                    and n not in _behind]
        for name, b in sorted(drawable, key=lambda kv: kv[1]['layer']):
            if name not in world:
                continue
            poly = to_world(b['mesh'], world[name])
            _stroke(poly, b['color'], b['outline'], _bone_w(name))
        ...
        if 'head' in world:
            ...
            # the face.
            _line = _pal2.get('outline', '#333333')
            _lw_face = max(1, int(round(hh * 0.055)))


            _fs = min(hh, hl * 0.95)          # face scale
            mz = hp(hl*0.98, hh*0.125); mr = _fs * 0.54
            d.ellipse([int(mz[0]-mr), int(mz[1]-mr*0.68), int(mz[0]+mr), int(mz[1]+mr*0.68)],
                      fill=_pal2.get('muzzle', '#5A5A5A'), outline=_line,
                      width=_lw_face)

            # Nose
            _nr = max(2.5, _fs * 0.20)
            nsp = hp(hl*1.16, hh*0.083)
            d.ellipse([int(nsp[0]-_nr), int(nsp[1]-_nr*0.74),
                       int(nsp[0]+_nr), int(nsp[1]+_nr*0.74)], fill='#1A1A1A')
            _nh = hp(hl*1.16 - _nr*0.30, hh*0.083 - _nr*0.34)
            _nhr = max(1, int(_nr*0.26))
            d.ellipse([int(_nh[0]-_nhr), int(_nh[1]-_nhr),
                       int(_nh[0]+_nhr), int(_nh[1]+_nhr)], fill='#8A8A8A')

            # Eye: sclera, iris and a catchlight.
            eye = hp(hl*0.58, -hh*0.33)
            _er = max(3, int(hh*0.217))
            disc(eye, _er, 'white', '#222222', 1)
            disc(eye, max(2, int(_er*0.55)), '#1A1A1A', None, 0)
            _hl_off = _er * 0.34
            _hp_ = hp(hl*0.58 + _hl_off*0.55, -hh*0.33 - _hl_off*0.85)
            disc(_hp_, max(1, int(_er*0.26)), '#FFFFFF', None, 0)
...
def _(mo):
    mo.md(r"""
    ## Checks and a still preview


    Solves every frame and checks the result against the reference gait: the phase lag of each limb relative to the left foreleg, and how much of the time the elbow and stifle sit against their limits. Then draws one frame, so the character can be judged before a whole video is rendered.
    """)
    return
...
def _(
    ...
    def preview(idx=0):
        n_pv = len(stab_kps)
        if idx > n_pv - 1:
            print(f"preview: frame {idx} requested but the clip has {n_pv} "
                  f"frames; showing frame {n_pv - 1} instead")
            idx = n_pv - 1
        idx = max(0, idx)
...
def _(mo):
    ...
    Renders every frame and writes the three-panel comparison: the source footage, the tracked skeleton, and the character, playing in step.
    """)
    return
...
def _(
    ...
    def generate_video(vid_path=None, out_path='rig_v2.mp4',
                       fps=30, panel_h=400, anim_w=600):
        vid_path = vid_path or VIDEO_PATH
    ...
    import os as _osv
    _stem = _osv.path.splitext(_osv.path.basename(VIDEO_PATH))[0]
    generate_video(out_path=f'outputs/{_stem}_rig.mp4')
    return
...
def _(mo):
    ...
    Everything below this point inspects the result. Nothing here affects the render, and the cells can be run in any order or not at all.

    These exist because several problems in this project looked like one thing and turned out to be another: a twitch that was two separate causes, a torso seam that was a mismatched constant. Each cell isolates one measurable quantity.
    """)
    return
...
def _(
    ...
    _i60 = min(60, len(stab_kps) - 1)
    p = solve_pose(stab_kps[_i60], rig, _i60, root_y=root_y_series[_i60] - aline_info['root_lift_px'], fold_ref=spine_fold_ref)
    print("arch:", p['arch'])
...
def _(
    ...
    _j60 = min(60, len(stab_kps) - 1)
    p60 = solve_pose(stab_kps[_j60], rig, _j60, root_y=root_y_series[_j60] - aline_info['root_lift_px'], fold_ref=spine_fold_ref)
...
def _(mo):
    ...
    Longer checks that report a verdict rather than a number: whether the spine arch leads the gait correctly, whether the timing of the footfalls survives retargeting, and where any remaining twitch comes from.
    """)
    return
...
def _(
    ...
    def arch_audit():
        ...
        above = mid_k[1] < chord_y          # smaller y is higher on screen
...
def _(arch_audited, facing_series, get_kp, np, stab_kps):
    def timing_audit():
        ...
        # light smoothing; the raw paw x is noisy frame to frame
        k = np.ones(5) / 5.0
        spread = np.convolve(spread, k, mode='same')
        gather = -spread                      # large = gathered
...
def _(mo):
    ...
    Smaller one-off measurements kept from specific investigations — joint saturation, shoulder travel, bend distributions and gait phase. They are the working behind numbers quoted in the write-up.
    """)
    return
...
def _(arch_audit_raw, arch_trust_series, np, rig, solve_pose, stab_kps):
    ...
    for i_aa in range(len(stab_kps)):
        ...
        # signed distance of the middle vertex from the chord = arc height
...
def _(compute_world_transforms, leg_table, np, rig, solve_pose, stab_kps):
    ...
    # how often each nonlinearity is riding its boundary
    print()
    for key_la, lim_la, lbl_la in (('rad_L_rel', (-0.35, 1.60), 'elbow L'),
        ...
        span_la = (lim_la[1] - lim_la[0]) * 0.5      # clamp_soft softness
...
def _(arch_audit_raw, get_kp, np, rig, solve_pose, stab_kps):
    ...
    P_pa[f_pa < 1.0 / 60] = 0.0          # ignore camera drift
...
def _(mo):
    ...
    Renders the character at several frames side by side, which makes a problem that only appears in certain poses easier to see than scrubbing through a video.
    """)
    return
...
def _(
    ...
    import json as _gj
    import os as _gos
    from posetoon_aline import build_character_rig as _bcr

    GALLERY_FRAMES = [60, 150]

    GALLERY_FRAMES = sorted({min(_f, len(stab_kps) - 1) for _f in GALLERY_FRAMES})
    GALLERY_COMPLEXITY = 'fine'
    _gos.makedirs('outputs', exist_ok=True)

    with open('outputs/breed_templates.json') as _f:
        _names = sorted(_gj.load(_f)['templates'].keys())
    print(f"breed templates found: {len(_names)}  ->  {', '.join(_names)}")

    _tiles = []
    for _fr_i in GALLERY_FRAMES:
        for _nm in _names:
            _rg, _inf = _bcr(measure, n_frames=len(stab_kps),
                             force_template=_nm,
                             complexity=GALLERY_COMPLEXITY, verbose=False)
            _p = solve_pose(stab_kps[_fr_i], _rg, _fr_i,
                            root_y=root_y_series[_fr_i] - _inf['root_lift_px'],
                            fold_ref=spine_fold_ref)
            if _p is None:
                print(f"  !! {_nm} skipped at frame {_fr_i} (no pose)")
                continue
            _w = compute_world_transforms(_rg, _p)
            _img = render_rig(_rg, _w, _p).convert('RGB')
            _tiles.append((_nm, _fr_i, _img))
            if _fr_i == GALLERY_FRAMES[0]:
                _leg = _inf['measure_used']['fem'] + _inf['measure_used']['tib']
                print(f"  {_nm:14} hind leg {_leg:5.1f}px  "
                      f"root lift {_inf['root_lift_px']:4.1f}px  "
                      f"ear={_inf['appearance']['ear_type']:11}"
                      f"  colour={_inf['colour_source']}")

    # Wrapped into a grid, with the breed name drawn on each tile.

    if _tiles:
        PER_ROW = 4
        _H = min(im.height for _, _, im in _tiles)
        _W = min(im.width for _, _, im in _tiles)
        _nr = -(-len(_tiles) // PER_ROW)
        _sheet = Image.new('RGB', (PER_ROW * _W, _nr * _H), (240, 240, 240))
        _dr = ImageDraw.Draw(_sheet)
        for _k, (_nm, _fr_i, _im) in enumerate(_tiles):
            _r, _c = divmod(_k, PER_ROW)
            _sheet.paste(_im.crop((0, 0, _W, _H)), (_c * _W, _r * _H))
            _dr.text((_c * _W + 8, _r * _H + 8),
                     f"{_nm}  (frame {_fr_i})", fill=(30, 30, 30))
        _sheet.save('outputs/breed_gallery.png')
        print(f"\nsaved outputs/breed_gallery.png  "
              f"({len(_tiles)} tiles, {PER_ROW} per row, "
              f"{len(GALLERY_FRAMES)} frame(s) per template)")
    else:
        print("\n!! no tiles rendered -- check that solve_pose returned a pose")
    return
